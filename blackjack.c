...
void play(){
    printf("You are given 700 credits.\n");
    balance = 700;
    while(balance>0){
        bet();
        initdeck();
    ...
    if(ptotal < 21){
        ...
        while(dtotal < 16){
            ...
            if(dtotal < 16){
                ...
                printf("Oh no! The dealer just got 21.\n");
                break;
            }else if(dtotal > 21){
        ...
        if(dtotal<ptotal){
            ...
            printf("Its a tie! 1-to-1 payout of %d.\n", pot);
            balance += pot;
        }else if(dtotal>ptotal && dtotal <= 21){
            printf("Oh no! The dealer won this round. Try again!\n");   
        }else{
            printf("You beat the dealer! Your payout is %d.\n", (int)(pot*1.5)); 
            balance += (pot*1.5);
        }
...
void quit(){
    ...
    if(strcmp(input,"y")==0)
        ...
        printf("Quit has been cancelled.\n");
}
...
void bet(){
    ...
    while(val > balance || (val < 10  && val > 0) || val <=0){
        if(val > balance)
            printf("You don't enough credits. Your balance is: %d.\n", balance);
        else if(val <= 0)
            printf("Enter a valid bet amount. Type \"bet [amount]\"\n");
        else
